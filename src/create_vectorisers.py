...
team_id_vectoriser = HashingVectorizer(n_features=len(home_team_id_list), binary=True)
...
with open(os.path.join(sys.argv[2], params['team_vct_name']), "wb") as fd:
    pickle.dump(team_id_vectoriser, fd)


with open(os.path.join(sys.argv[2], params['city_vct_name']), "wb") as fd:
